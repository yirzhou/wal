...
func getSegmentIDFromWalFileName(fileName string) (uint64, error) {
	segmentId := strings.TrimPrefix(fileName, "wal-")
...
func getSegmentIDFromSegmentFileName(fileName string) (uint64, error) {
	segmentId := strings.TrimPrefix(fileName, "segment-")
	segmentIdInt, err := strconv.ParseUint(segmentId, 10, 64)
	if err != nil {
		return 0, err
	}
...
func getSSTableFileNameFromSegmentId(segmentId uint64) string {
	return fmt.Sprintf("segment-%06d", segmentId)
}
...
func (kv *KVStore) getNewSegmentFilePath() string {
	...
	segmentFileName := getSSTableFileNameFromSegmentId(lastSegmentID + 1)
...
func getWalFileNameFromSegmentId(segmentId uint64) string {
	return fmt.Sprintf("wal-%06d", segmentId)
}
...
func recoverFromWALs(lastSegmentID uint64, walDir string, memState *MemState) (*WAL, error) {
	...
	for idx, walFile := range walFiles {
		...
		if segmentID > lastSegmentID {
			...
			// Open the WAL file.
			walFile, err := os.OpenFile(filepath.Join(walDir, walFile), os.O_RDWR|os.O_CREATE|os.O_APPEND, 0644)
			if err != nil {
			...
			lastSequenceNum, err := processWALFile(walFile, memState, isLastWal)
			if err != nil {
...
// processWALFile processes a WAL file and returns the last sequence number.
// It also updates the in-memory state.
// If the WAL file is the last one, it truncates the file to the last good offset.
func processWALFile(reader *os.File, memState *MemState, isLastWal bool) (uint64, error) {
...
func NewKVStore(dir string) (*KVStore, error) {
	...
	// 1. Read the contents of the directory 'dir'.
	// 2. Find all files matching the "wal-xxxx" pattern.
	// 3. Parse the numbers to find the highest existing segment ID.
	//    - If no files exist, start with ID = 1.
	//    - Otherwise, use the highest found ID.
	// 4. Construct the path to this latest segment file.
	// 5. Open it for appending.
	// 6. Create and return the Log struct, populated with the dir,
	//    activeFile, activeSegmentID, and segmentSize.

	// First, look for the highest segment ID from a pile of existing WAL files.
	// walDir := filepath.Join(dir, "wal")
	// walFiles, err := listWalFiles(walDir)
	// if err != nil {
	// 	log.Println("Error listing WAL files:", err)
	// 	return nil, err
	// }
	// var highestSegmentID uint64
	// if len(walFiles) == 0 {
	// 	highestSegmentID = 1
	// } else {
	// 	highestSegmentID = getHighestSegmentId(walFiles)
	// }

	// walFileName := getWalFileNameFromSegmentId(highestSegmentID)
	...
	// // 2. Create the initial WAL object. The sequence number will be recovered from the file.
	// wal := &WAL{
	// 	activeFile:      file,
	// 	activeSegmentID: highestSegmentID,
	// 	dir:             walDir,
	// 	segmentSize:     CheckpointSize,
	// }

	// reader := wal.activeFile
	// var goodOffset int64 = 0
